...
describe "OAuth" do
  ...
  context 'refresh_access_token' do
    it "should fail without a client_id and secret" do
      token_api
      expect { @api.refresh_access_token(nil, nil) }.to raise_error(StandardError, 'client_id required for oauth flow')
    end

    it "should fail without a refresh token" do
      client_api
      expect { @api.refresh_access_token(nil, nil) }.to raise_error(StandardError, 'refresh_token required')
    end

    it "should fail without a redirect uri" do
      client_api
      expect { @api.refresh_access_token('token', nil) }.to raise_error(StandardError, 'redirect_uri required')
    end

    it "should fail on an invalid callback_url" do
      client_api
      expect { @api.retrieve_access_token("abc", nil) }.to raise_error(StandardError, 'redirect_uri required')
      expect { @api.retrieve_access_token("abc", "(*&^%$#") }.to raise_error(StandardError, 'invalid redirect_uri')
    end

    it "should successfully retrieve an access token" do
      client_api
      expect(@api).to receive(:post).and_return({'access_token' => 'asdf'})
      res = @api.refresh_access_token("abc", "http://www.example.com")
